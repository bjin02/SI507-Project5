...
#--------------------------------------------------
def writePostDataToCSVFile(fileName, data):
    with open(fileName, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["date", "summary", "format", "short_url", "can_like", "can_reply", "type"])
        posts = data["response"]["posts"]
        for post in posts:
            writer.writerow([post["date"], post["summary"], post["format"], post["short_url"], post["can_like"], post["can_reply"], post["type"]])

def writeInfoDataToCSVFile(fileName, data):
    with open(fileName, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["followed", "likes", "total_posts", "title", "url", "ask_page_title", "name"])
        blog = data["response"]["blog"]
        writer.writerow([blog["followed"], blog["likes"], blog["total_posts"], blog["title"], blog["url"], blog["ask_page_title"], blog["name"]])
...
if __name__ == "__main__":
    ...
    tumblr_params = {'blog-identifier': "peacecorps.tumblr.com", 'api':"/info"}

    tumblr_result = get_data_from_api(tumblr_baseurl,
                                      tumblr_params)
    writeInfoDataToCSVFile("infoData.csv", tumblr_result)
    ...
    tumblr_result = get_data_from_api(tumblr_baseurl,
                                      tumblr_params)
    writePostDataToCSVFile("postData.csv", tumblr_result)
